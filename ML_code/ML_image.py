...
dir="C:\\Rouhin_Lenovo\\DC_summer_school\\Data\\Image_test\\Landsat_test\\EarthEngineImages"
os.chdir(dir)
...
class landsat_gee(RasterDataset):
    ...
    def plot(self, sample):
        ...
        #     print("NaN values found in the image data")
    ...
    def inference(self,model,image):
        ...
        '''
        pred=model.forward(image)/28.36
        # print(pred.reshape(512,512))
        # print(pred.transpose(0,1).reshape(512,512).detach().numpy())
        # print("pred shape",np.argwhere(np.isnan(pred.reshape(512,512).detach())))
        self.plot_other(pred.reshape(512,512).detach(),0,3)
        return np.argwhere(np.isnan(pred.reshape(512,512).detach()))

torch.manual_seed(7)
dataset = landsat_gee(dir)
sampler = RandomGeoSampler(dataset, size=512, length=1)
...
for batch in dataloader:
    ...
    # print(a.shape)
    # tmp=a.reshape(512,512).detach().numpy()
    dataset.plot_other(sample["image"][18],0,5)
    plt.axis("off")
    plt.show()
...
model.load_state_dict(torch.load("C:\\Rouhin_Lenovo\\DC_summer_school\\Data\\ML_models\\Phys5000.pt"))
...
# And now to load...
scaler = joblib.load("C:\\Rouhin_Lenovo\\DC_summer_school\\Data\\ML_models\\standard_scaler_phys.bin") 
# print(model.eval())
for batch in dataloader:
    sample = unbind_samples(batch)[0]
    print(sample["image"][18])
